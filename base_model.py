...
def Cos_si(features):
    with tf.name_scope('Cosine_Similarity'):
        # Cosine similarity
        query_norm = tf.sqrt(tf.reduce_sum(tf.square(features[0]), 1, True))
        doc_norm = tf.sqrt(tf.reduce_sum(tf.square(features[1]), 1, True))

        prod = tf.reduce_sum(tf.multiply(features[0], features[1]), 1, True)
        norm_prod = tf.multiply(query_norm, doc_norm)

        cos_sim_raw = tf.truediv(prod, norm_prod)
        cos_sim = tf.squeeze(cos_sim_raw, [1])
        return cos_sim
def Eru(features):
    with tf.variable_scope('EuclideanDis'):
        dis = tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(features[0], features[1])), 1))
        return dis
def Man(features):
    with tf.variable_scope('Man'):
        sub = tf.abs(tf.subtract(features[0], features[1]))
        return sub
def Mul(features):
    with tf.variable_scope('Mul'):
        mul = tf.multiply(features[0], features[1])
        return mul
...
def fc_layer(feature, num_class, l2_loss, scope, TRAIN):
  with tf.variable_scope(scope):
    ...
    fc1_out, fc1_loss = linear(feature, 512, TRAIN, scope='fc1', stddev=0.1)
    l2_loss += fc1_loss
    
    fc2_out, fc2_loss = linear(tf.nn.relu(fc1_out), 1024, TRAIN, scope='fc2', stddev=0.1)
    l2_loss += fc2_loss
    
    fc3_outs, fc3_loss = linear(tf.nn.relu(fc2_out), num_class, TRAIN, scope='fc3', stddev=0.1)
    #fc3_outs = tf.layers.batch_normalization(fc3_outs, trainable=TRAIN)
    #fc3_outs = tf.nn.sigmoid(fc3_outs)

    #fc3_out, fc3_loss = linear(tf.nn.relu(feature), num_class, TRAIN, scope='fc3', stddev=0.1)
    l2_loss += fc3_loss
    return fc3_outs, l2_loss
